...
void Interno() {
 ...
 for (int i = 13; i < 23; i++) {  // borde
   leds6.setPixelColor(i, 0, 0, 30 / multiple_test);
  
  }

   for (int i = 0; i < 10; i++) {  // borde
    led_temp = led_flash + i;
    if (led_temp < 10) {
      leds6.setPixelColor(13 + led_temp, 0, 0, (10 - i) * 8/ multiple_test);
    } else {
      leds6.setPixelColor(13 + led_temp - 10, 0, 0, (10 - i) * 8/ multiple_test);
    }
  }
  led_flash -= 1;
  if (led_flash < 0) {
 ...
 for (int i = 23; i < NUMLEDS6; i++) {  // borde
